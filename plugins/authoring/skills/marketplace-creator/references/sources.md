# Sources

Canonical documentation this skill is built from.

**Last verified:** 2026-07-07 against the live docs below. Agent plugins are a Preview feature, so there is no stable version to pin. Re-verify against these pages if behavior differs.

## VS Code

- https://code.visualstudio.com/docs/agent-customization/agent-plugins

## GitHub Copilot CLI

- https://docs.github.com/en/copilot/how-tos/copilot-cli/customize-copilot/plugins-marketplace
- https://docs.github.com/en/copilot/how-tos/copilot-cli/customize-copilot/plugins-finding-installing
- https://docs.github.com/en/copilot/reference/copilot-cli-reference/cli-plugin-reference
- https://docs.github.com/en/copilot/reference/copilot-cli-reference/cli-plugin-reference#marketplacejson-fields

## Claude Code

- https://code.claude.com/docs/en/plugin-marketplaces
