...
- Default council = 3 cross-vendor **roles**, latest of each: **Opus** (Anthropic flagship) + **Gemini Pro** (Google) + the **latest GPT** (OpenAI) — today Opus 4.8 + Gemini 3.1 Pro (Preview) + GPT-5.6 Sol; add a code specialist (MAI-Code) 4th for code-heavy work. Drop to the **workhorse** tier (Sonnet / Gemini 2.5 Pro / GPT-5.5) for cheaper, faster fan-out. Cross-vendor is *preferred, not required* — a same-vendor multi-tier council (Opus + Sonnet + Haiku) is a valid fallback and the only option on single-vendor harnesses; it shares that vendor's blind spots, so weight its agreement accordingly.
