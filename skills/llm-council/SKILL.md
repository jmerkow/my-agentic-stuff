...
- Default council = 3 cross-vendor **roles** at the latest tier: an Anthropic flagship (Opus) + a Google Pro + an OpenAI current model — today Claude Opus 4.8 + Gemini 3.1 Pro (Preview) + GPT-5.6 Sol (its flagship-tier variant); add a code-specialist 4th for code-heavy work. Drop to mid-tier (Sonnet 5 / Gemini 2.5 Pro / GPT-5.5) for cheaper, faster fan-out. Cross-vendor is *preferred, not required* — a same-vendor multi-tier council (e.g. Opus + Sonnet + Haiku) is a valid fallback and the only option on single-vendor harnesses; it shares that vendor's blind spots, so weight its agreement accordingly.
